...
def __(temperature_slider_ucst):
    ...
    # Interaction parameters (different from LCST)
    omega_alpha_ucst = 12000  # J/mol
    omega_beta_ucst = 20000   # J/mol
    
    # Temperature-dependent reference energies for UCST behavior
    # Note the positive temperature coefficients!
    G0_A_alpha_ucst = 0
    G0_B_alpha_ucst = -3000 + 8 * T_ucst  # Increases with T
    G0_A_beta_ucst = -2000 + 6 * T_ucst   # Increases with T
    G0_B_beta_ucst = -1000 + 7 * T_ucst   # Increases with T
...
def __(G0_A_alpha_ucst, G0_A_beta_ucst, G0_B_alpha_ucst, G0_B_beta_ucst, R_ucst, T_ucst, mo, np, omega_alpha_ucst, omega_beta_ucst):
    ...
    **UCST Parameters:**
    ...
    - G°_B^α = -3.0 + 8T kJ/mol = {G0_B_alpha_ucst/1000:.1f} kJ/mol
    - G°_A^β = -2.0 + 6T kJ/mol = {G0_A_beta_ucst/1000:.1f} kJ/mol
    - G°_B^β = -1.0 + 7T kJ/mol = {G0_B_beta_ucst/1000:.1f} kJ/mol
...
def __(G_alpha_ucst, G_beta_ucst, GridSpec, Poly3DCollection, R_ucst, T_ucst, fsolve, mo, np, plt, tangent_found_ucst, x1_tangent_ucst, x2_tangent_ucst, omega_alpha_ucst, omega_beta_ucst):
    ...
    def calc_G_beta_3d_ucst(x, temp):
        if x <= 0 or x >= 1:
            return np.inf
        G0_A_beta_temp = -2000 + 6 * temp
        G0_B_beta_temp = -1000 + 7 * temp
        G_mix = R_ucst * temp * (x * np.log(x) + (1-x) * np.log(1-x))
        G_ref = x * G0_B_beta_temp + (1-x) * G0_A_beta_temp
        G_excess = x * (1-x) * omega_beta_ucst
        return G_ref + G_mix + G_excess
    ...
    for temp_ucst in temperatures_shadow_ucst:
        ...
        if temp_ucst < 400:
            ...
            # Linear interpolation for inverted dome
            progress = (temp_ucst - 400) / 700
            x1 = 0.1 + 0.35 * progress
            x2 = 0.9 - 0.35 * progress
            if x2 - x1 < 0.1:  # Phases merge
                x1 = None
                x2 = None
    ...
    return ax_2d_ucst, ax_3d_ucst, ax_phase_ucst, calc_G_alpha_3d_ucst, calc_G_beta_3d_ucst, fig_ucst, gs_ucst, valid_temps_shadow_ucst, x1_values_shadow_ucst, x2_values_shadow_ucst, z_bottom_ucst
